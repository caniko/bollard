...
impl Docker {
    ...
    ///
    /// On Unix, the discovery order depends on enabled feature flags:
    ///
    /// - **`podman` feature (default when enabled):** `$DOCKER_HOST` → rootless
    ///   Podman → system Podman → Docker system socket.
    /// - **`docker` feature only:** `$DOCKER_HOST` → Docker system socket
    ///   (`/var/run/docker.sock`).
    ///
    /// When both `podman` and `docker` features are enabled, the `podman`
    /// discovery order takes precedence (Podman sockets are tried first).
    ///
    /// On Windows, it delegates to `connect_with_named_pipe_defaults`.
    pub fn connect_with_local_defaults() -> Result<Docker, Error> {
        #[cfg(all(unix, feature = "podman"))]
        return Docker::connect_with_podman_defaults();
        #[cfg(all(unix, feature = "docker", not(feature = "podman")))]
        return Docker::connect_with_unix_defaults();
        #[cfg(all(unix, not(feature = "docker"), not(feature = "podman")))]
        return Docker::connect_with_unix_defaults();
        #[cfg(windows)]
...
impl Docker {
    ...
    fn podman_rootless_socket_path() -> Option<String> {
        ...
        #[cfg(unix)]
        {
            ...
            if let Ok(meta) = std::fs::metadata("/proc/self") {
                let sock = DEFAULT_PODMAN_SOCKET_TEMPLATE
                    .replace("{UID}", &meta.uid().to_string());
                if Path::new(&sock).exists() {
                    return Some(sock);
                }
            }
    ...
    fn podman_system_socket_path() -> Option<&'static str> {
        let path = DEFAULT_PODMAN_SYSTEM_SOCKET.strip_prefix("unix://").unwrap_or(DEFAULT_PODMAN_SYSTEM_SOCKET);
